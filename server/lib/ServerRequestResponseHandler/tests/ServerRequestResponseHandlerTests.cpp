...
GTEST_TEST(ServerRequestResponseHandlerTests, SignalRequest_ReturnsSignalResponseWithCorrectData) {
    ...
    creator->addMessageOnMap("signal", [](const std::string& type, Transaction t, boost::json::value& jv) -> std::unique_ptr<Message> {
        return std::make_unique<SignalMessage>(type,Transaction::Response, jv);
    });
...
GTEST_TEST(ServerRequestResponseHandlerTests, InformationRequest_ReturnsInformationResponseWithCorrectData) {
    auto creator = std::make_shared<CreatorMessage>();
    creator->addMessageOnMap("information", [](const std::string& type, Transaction t, boost::json::value& jv) -> std::unique_ptr<Message> {
        return std::make_unique<InformationMessage>(type,t, jv);
    });
    ServerRequestResponseHandler handler(creator);

    auto request = std::make_unique<Message>("information",Transaction::Request);
...
GTEST_TEST(ServerRequestResponseHandlerTests, UnknownRequestType_ReturnsNullptr) {
    ...
    auto request = std::make_unique<Message>("unknown",Transaction::Request);
