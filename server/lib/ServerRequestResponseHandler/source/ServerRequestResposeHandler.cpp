...
std::unique_ptr<Message> ServerRequestResponseHandler::processingRequestResponse(std::unique_ptr<Message> message) {
    ...
    if (message->transactionType == TransactionType::Request) {
        logger("DEBUG") << "ServerRequestResponseHandler : message->transactionType:" << static_cast<int>(message->transactionType) << "\n";
        json::value jv;
    ...
    logger("ERROR") << "ServerRequestResponseHandler : message->transactionType:" << static_cast<int>(message->transactionType) << "\n";
    return nullptr;
};