...
public:
	CreatorMessage();
	CreatorMessage(std::unordered_map<
	               std::string, std::function<std::unique_ptr<Message>(const std::string&, Transaction, json::value&)>>
	                   factory);

	std::unique_ptr<Message> createMessage(const std::string& type, Transaction transaction, json::value& json_value);

	bool
	addMessageOnMap(const std::string& type,
