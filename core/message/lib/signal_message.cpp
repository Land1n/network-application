...
SignalMessage::SignalMessage(const std::string& type, Transaction transaction, const int central_Freq,
                             std::vector<std::complex<float>> signal) :
    Message(type, transaction), _central_Freq(central_Freq), _signal(std::move(signal))
{}

SignalMessage::SignalMessage(const std::string& type, Transaction transaction, json::value& jv) :
	...
	if(transaction == Transaction::Response) {
		...
		for(auto const& value: signalArray) {
			if(value.is_array() && value.as_array().size() == 2) {
				...
				_signal.emplace_back(real, imag);
			}
		}
	}
}

int SignalMessage::getCentralFreq() const
{
	return _central_Freq;
}

std::vector<std::complex<float>> SignalMessage::getSignal() const
{
	return _signal;
}
