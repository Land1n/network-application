...
ConnectHandler::ConnectHandler(std::shared_ptr<tcp::socket>& socket) : socket(socket)
{}
...
void ConnectHandler::connect(const tcp::endpoint& endpoint, IOMode mode)
{
	if(mode == IOMode::Sync)
...
void ConnectHandler::connect(const std::string& host, PortInt port, IOMode mode)
{
	auto endpoint = tcp::endpoint(boost::asio::ip::make_address(host), port);
	connect(endpoint, mode);
}

void ConnectHandler::sync_connect(const tcp::endpoint& endpoint)
{
	error_code error;
...
void ConnectHandler::async_connect(const tcp::endpoint& endpoint)
{
...
void ConnectHandler::disconnect()
{
	if(!socket->is_open())
		return;
...
void ConnectHandler::setOnConnect(const CallbackError& c)
{
	onConnect = c;
}
void ConnectHandler::setOnDisconnect(const CallbackError& c)
{
	onDisconnect = c;
}
void ConnectHandler::setOnError(const CallbackError& c)
{
	onError = c;
}