//
// Created by ivan on 07.03.2026.
//
#include <gtest/gtest.h>
#include <boost/asio.hpp>
#include <thread>
#include <boost/json.hpp>
#include <future>
#include <atomic>
#include <mutex>

#include "transport_handler/transport_handler.h"
#include "io_context_handler/io_context_handler.h"
#include "connection/connect_handler.h"
#include "connection/accept_handler.h"
#include "message_handler/message_handler.h"
#include "message/information_message.h"
#include "logger/logger.h"

namespace json = boost::json;

class TransportHandlerTests : public ::testing::Test {
public:
	tcp::endpoint endpoint = tcp::endpoint(boost::asio::ip::make_address("127.0.0.1"), 8090);

	IOContextHandler server_context;
	IOContextHandler client_context;

	std::shared_ptr<tcp::socket> client_socket = std::make_shared<tcp::socket>(client_context.getIOContext());
	std::shared_ptr<tcp::socket> server_socket = std::make_shared<tcp::socket>(server_context.getIOContext());

	ConnectHandler connection_handler{client_socket};
	AcceptHandler accept_handler{server_context.getIOContext(), endpoint};

	void TearDown() override
	{
		boost::system::error_code ec;
		server_socket->shutdown(boost::asio::ip::tcp::socket::shutdown_both, ec);
		server_socket->close(ec);
		server_socket.reset();
		client_socket.reset();
	}

	TransportMessage create_test_message()
	{
		std::string json_str = R"({"type":"test","transaction":2})";
		std::vector<uint8_t> payload(json_str.begin(), json_str.end());
		return TransportMessage("test", Transaction::Tests, payload);
	}

	TransportMessage create_info_message(int id)
	{
		auto message = std::make_unique<InformationMessage>("information", Transaction::Response, id);
		MessageHandler handler;
		return handler.serialize(std::move(message));
	}

	int get_data_message(TransportMessage&& transportMessage)
	{
		MessageHandler handler;
		auto message      = handler.parse(std::move(transportMessage));
		auto info_message = dynamic_cast<InformationMessage*>(message.get());
		return info_message->getNumberCore();
	}

	static void expect_success_with_tm(error_code ec, TransportMessage tm)
	{
		EXPECT_FALSE(ec);
	}
	static void expect_success(error_code ec)
	{
		EXPECT_FALSE(ec);
	}
	static void expect_failure(error_code ec, TransportMessage tm)
	{
		EXPECT_TRUE(ec);
	}
};

TEST_F(TransportHandlerTests, SyncWriteSuccessful)
{
	std::thread t([&]() {
		accept_handler.accept(server_socket, IOMode::Sync);
	});
	connection_handler.connect(endpoint, IOMode::Sync);
	TransportHandler transport_handler(client_socket);

	transport_handler.setOnAllWrite(expect_success_with_tm);
	transport_handler.write(create_test_message(), IOMode::Sync);
	if(t.joinable())
		t.join();
}

TEST_F(TransportHandlerTests, AsyncWriteSuccessful)
{
	std::promise<void> accept_done, connect_done, write_done;

	TransportHandler transport_handler(client_socket);
	transport_handler.setOnAllWrite([&](error_code ec, TransportMessage tm) {
		EXPECT_FALSE(ec);
		write_done.set_value();
	});
	connection_handler.setOnConnect([&](error_code error) {
		transport_handler.write(create_test_message(), IOMode::Async);
		EXPECT_FALSE(error);
		connect_done.set_value();
	});

	accept_handler.setOnAccept([&](error_code error) {
		EXPECT_FALSE(error);
		accept_done.set_value();
	});

	connection_handler.connect(endpoint, IOMode::Async);
	accept_handler.accept(server_socket, IOMode::Async);
	accept_done.get_future().wait();
	connect_done.get_future().wait();
	write_done.get_future().wait();
}

TEST_F(TransportHandlerTests, SyncWriteAndReadSuccessful)
{
	connection_handler.setOnConnect(expect_success);
	accept_handler.setOnAccept(expect_success);

	std::thread server([&]() {
		accept_handler.accept(server_socket, IOMode::Sync);
		TransportHandler transport_handler_server(server_socket);
		transport_handler_server.setMagicNumber(12345);
		transport_handler_server.setOnAllRead(
		    [for_write_message = create_test_message()](error_code error, TransportMessage&& for_read_message) {
			    EXPECT_FALSE(error);
			    EXPECT_EQ(for_read_message.type, for_write_message.type);
			    EXPECT_EQ(for_read_message.transaction, for_write_message.transaction);
			    EXPECT_EQ(for_read_message.payload, for_write_message.payload);
		    });
		transport_handler_server.read(IOMode::Sync);
	});
	connection_handler.connect(endpoint, IOMode::Sync);

	TransportHandler transport_handler(client_socket);
	transport_handler.setMagicNumber(12345);
	transport_handler.setOnAllWrite(expect_success_with_tm);
	transport_handler.write(create_test_message(), IOMode::Sync);
	server.join();
}

TEST_F(TransportHandlerTests, AsyncWriteAndReadSuccessful)
{
	std::promise<void> accept_done, connect_done, write_done, read_done;

	TransportHandler transport_handler_client(client_socket);
	TransportHandler transport_handler_server(server_socket);

	transport_handler_client.setOnAllWrite([&](error_code ec, TransportMessage&& tm) {
		EXPECT_FALSE(ec);
		write_done.set_value();
	});

	connection_handler.setOnConnect([&](error_code error) {
		transport_handler_client.write(create_test_message(), IOMode::Async);
		EXPECT_FALSE(error);
		connect_done.set_value();
	});
	transport_handler_server.setOnAllRead(
	    [&, for_write_message = create_test_message()](error_code error, TransportMessage for_read_message) {
		    EXPECT_FALSE(error);
		    EXPECT_EQ(for_read_message.type, for_write_message.type);
		    EXPECT_EQ(for_read_message.transaction, for_write_message.transaction);
		    EXPECT_EQ(for_read_message.payload, for_write_message.payload);
		    read_done.set_value();
	    });
	accept_handler.setOnAccept([&](error_code error) {
		EXPECT_FALSE(error);
		transport_handler_server.read(IOMode::Async);
		accept_done.set_value();
	});
	accept_handler.accept(server_socket, IOMode::Async);
	connection_handler.connect(endpoint, IOMode::Async);

	accept_done.get_future().wait();
	connect_done.get_future().wait();
	write_done.get_future().wait();
	read_done.get_future().wait();
}

TEST_F(TransportHandlerTests, SyncWriteAndError)
{
	std::atomic<bool> isError = false;

	TransportHandler transport_handler(client_socket);
	transport_handler.setOnError([&isError](error_code error) {
		isError.store(true);
	});
	transport_handler.write(create_test_message(), IOMode::Sync);
	EXPECT_TRUE(isError.load());
}

TEST_F(TransportHandlerTests, AsyncWriteAndError)
{
	std::promise<void> error_done;
	std::atomic<bool> isError = false;

	TransportHandler transport_handler(client_socket);
	transport_handler.setOnError([&](error_code error) {
		isError = true;
		error_done.set_value();
	});
	transport_handler.write(create_test_message(), IOMode::Async);
	error_done.get_future().wait();
	EXPECT_TRUE(isError.load());
}

TEST_F(TransportHandlerTests, SyncWriteNAndReadN)
{
	Logger::getInstance().setLevel(LogLevel::NoLog);
	const short N = 100;
	std::atomic<short> N_write{0};
	std::atomic<short> N_read{0};
	std::vector<int> ints;
	std::mutex ints_mutex;

	accept_handler.setOnAccept([](error_code ec) {
		EXPECT_FALSE(ec);
	});
	connection_handler.setOnConnect([](error_code ec) {
		EXPECT_FALSE(ec);
	});

	std::thread server_thread([&]() {
		accept_handler.accept(server_socket, IOMode::Sync);
		TransportHandler th_server(server_socket);
		th_server.setMagicNumber(12345);
		th_server.setOnAllRead([&](error_code ec, TransportMessage&& msg) {
			EXPECT_FALSE(ec);
			int u = get_data_message(std::move(msg));
			{
				std::lock_guard<std::mutex> lock(ints_mutex);
				auto it = std::find(ints.begin(), ints.end(), u);
				EXPECT_NE(it, ints.end());
			}
			N_read++;
		});
		for(int i = 0; i < N; ++i) {
			th_server.read(IOMode::Sync);
		}
	});

	connection_handler.connect(endpoint, IOMode::Sync);
	TransportHandler th_client(client_socket);
	th_client.setMagicNumber(12345);
	th_client.setOnAllWrite([&](error_code ec, TransportMessage) {
		EXPECT_FALSE(ec);
		{
			std::lock_guard<std::mutex> lock(ints_mutex);
			ints.push_back(N_write);
		}
		N_write++;
	});
	for(int i = 0; i < N; ++i) {
		th_client.write(create_info_message(i), IOMode::Sync);
	}

	server_thread.join();
	while(N_read < N) {
		std::this_thread::sleep_for(std::chrono::milliseconds(1));
	}
	EXPECT_EQ(N_read, N);
	EXPECT_EQ(N_write, N);
}

TEST_F(TransportHandlerTests, AsyncWriteNAndReadN)
{
	const short N = 100;
	std::atomic<short> N_write{0};
	std::atomic<short> N_read{0};
	std::vector<int> ints;
	std::mutex ints_mutex;

	TransportHandler th_client(client_socket);
	th_client.setMagicNumber(12345);
	TransportHandler th_server(server_socket);
	th_server.setMagicNumber(12345);

	th_client.setOnAllWrite([&](error_code error, TransportMessage) {
		EXPECT_FALSE(error);
		{
			std::lock_guard<std::mutex> lock(ints_mutex);
			ints.push_back(N_write);
		}
		N_write++;
		if(N_write < N) {
			th_client.write(create_info_message(N_write), IOMode::Async);
		}
	});

	th_server.setOnAllRead([&](error_code error, TransportMessage msg) {
		EXPECT_FALSE(error);
		int u = get_data_message(std::move(msg));
		{
			std::lock_guard<std::mutex> lock(ints_mutex);
			auto it = std::find(ints.begin(), ints.end(), u);
			EXPECT_NE(it, ints.end());
		}
		N_read++;
		if(N_read < N) {
			th_server.read(IOMode::Async);
		}
	});

	connection_handler.setOnConnect([&](error_code error) {
		EXPECT_FALSE(error);
		th_client.write(create_info_message(0), IOMode::Async);
	});
	accept_handler.setOnAccept([&](error_code error) {
		EXPECT_FALSE(error);
		th_server.read(IOMode::Async);
	});
	connection_handler.connect(endpoint, IOMode::Async);
	accept_handler.accept(server_socket, IOMode::Async);

	while(N_read < N) {
		std::this_thread::sleep_for(std::chrono::milliseconds(1));
	}
	EXPECT_EQ(N_read, N_write);
}

TEST_F(TransportHandlerTests, SyncOneReadTwoWrite)
{
	bool startRead = false;

	std::thread client_1([&]() {
		TransportHandler th_client(client_socket);
		th_client.setMagicNumber(12345);
		th_client.setOnAllWrite([&](error_code ec, TransportMessage) {
			EXPECT_FALSE(ec);
		});
		connection_handler.setOnConnect([&](error_code ec) {
			EXPECT_FALSE(ec);
			while(!startRead) {
				std::this_thread::sleep_for(std::chrono::milliseconds(1));
			}
			th_client.write(create_info_message(1), IOMode::Sync);
		});
		connection_handler.connect(endpoint, IOMode::Sync);
	});

	std::thread client_2([&]() {
		IOContextHandler client_context_2;
		std::shared_ptr<tcp::socket> client_socket_2 = std::make_shared<tcp::socket>(client_context_2.getIOContext());
		ConnectHandler connection_handler_2(client_socket_2);
		TransportHandler th_client(client_socket_2);
		th_client.setMagicNumber(12345);
		th_client.setOnAllWrite([&](error_code ec, TransportMessage) {
			EXPECT_FALSE(ec);
		});
		connection_handler_2.setOnConnect([&](error_code ec) {
			EXPECT_FALSE(ec);
			while(!startRead) {
				std::this_thread::sleep_for(std::chrono::milliseconds(1));
			}
			th_client.write(create_info_message(2), IOMode::Sync);
		});
		connection_handler_2.connect(endpoint, IOMode::Sync);
	});

	short c_accept = 0;
	accept_handler.setOnAccept([&](error_code error) {
		EXPECT_FALSE(error);
		c_accept++;
		if(c_accept == 2) {
			startRead = true;
		}
	});

	std::thread server_socket_1([&]() {
		accept_handler.accept(server_socket, IOMode::Sync);
		TransportHandler th_server(server_socket);
		th_server.setMagicNumber(12345);
		th_server.setOnAllRead([&](error_code error, TransportMessage&& tm) {
			EXPECT_FALSE(error);
			int val = get_data_message(std::move(tm));
			EXPECT_TRUE(val == 1 || val == 2);
		});
		while(!startRead) {
			std::this_thread::sleep_for(std::chrono::milliseconds(1));
		}
		th_server.read(IOMode::Sync);
	});

	std::thread server_socket_2([&]() {
		std::shared_ptr<tcp::socket> server_socket_2 = std::make_shared<tcp::socket>(server_context.getIOContext());
		accept_handler.accept(server_socket_2, IOMode::Sync);
		TransportHandler th_server(server_socket_2);
		th_server.setMagicNumber(12345);
		th_server.setOnAllRead([&](error_code error, TransportMessage&& tm) {
			EXPECT_FALSE(error);
			int val = get_data_message(std::move(tm));
			EXPECT_TRUE(val == 1 || val == 2);
		});
		while(!startRead) {
			std::this_thread::sleep_for(std::chrono::milliseconds(1));
		}
		th_server.read(IOMode::Sync);
		server_socket_2->close();
	});

	client_1.join();
	client_2.join();
	server_socket_1.join();
	server_socket_2.join();
}

TEST_F(TransportHandlerTests, AsyncOneReadTwoWrite)
{
	bool startRead = false;

	std::thread client_1([&]() {
		std::promise<void> all_do;
		TransportHandler th_client(client_socket);
		th_client.setMagicNumber(12345);
		th_client.setOnAllWrite([&](error_code ec, TransportMessage) {
			all_do.set_value();
		});
		connection_handler.setOnConnect([&](error_code ec) {
			EXPECT_FALSE(ec);
			while(!startRead) {
				std::this_thread::sleep_for(std::chrono::milliseconds(1));
			}
			th_client.write(create_info_message(1), IOMode::Async);
		});
		connection_handler.connect(endpoint, IOMode::Async);
		all_do.get_future().wait();
	});

	std::thread client_2([&]() {
		std::promise<void> all_do;
		IOContextHandler client_context_2;
		std::shared_ptr<tcp::socket> client_socket_2 = std::make_shared<tcp::socket>(client_context_2.getIOContext());
		ConnectHandler connection_handler_2(client_socket_2);
		TransportHandler th_client(client_socket_2);
		th_client.setMagicNumber(12345);
		th_client.setOnAllWrite([&](error_code ec, TransportMessage) {
			all_do.set_value();
		});
		connection_handler_2.setOnConnect([&](error_code ec) {
			EXPECT_FALSE(ec);
			while(!startRead) {
				std::this_thread::sleep_for(std::chrono::milliseconds(1));
			}
			th_client.write(create_info_message(2), IOMode::Async);
		});
		connection_handler_2.connect(endpoint, IOMode::Async);
		all_do.get_future().wait();
	});

	short c_accept = 0;
	accept_handler.setOnAccept([&](error_code error) {
		EXPECT_FALSE(error);
		c_accept++;
		if(c_accept == 2) {
			startRead = true;
		}
	});

	std::thread server_socket_1([&]() {
		std::promise<void> all_do;
		accept_handler.accept(server_socket, IOMode::Async);
		TransportHandler th_server(server_socket);
		th_server.setMagicNumber(12345);
		th_server.setOnAllRead([&](error_code error, TransportMessage&& tm) {
			EXPECT_FALSE(error);
			int val = get_data_message(std::move(tm));
			EXPECT_TRUE(val == 1 || val == 2);
			all_do.set_value();
		});
		while(!startRead) {
			std::this_thread::sleep_for(std::chrono::milliseconds(1));
		}
		th_server.read(IOMode::Async);
		all_do.get_future().wait();
	});

	std::thread server_socket_2([&]() {
		std::promise<void> all_do;
		std::shared_ptr<tcp::socket> server_socket_2 = std::make_shared<tcp::socket>(server_context.getIOContext());
		accept_handler.accept(server_socket_2, IOMode::Async);
		TransportHandler th_server(server_socket_2);
		th_server.setMagicNumber(12345);
		th_server.setOnAllRead([&](error_code error, TransportMessage&& tm) {
			EXPECT_FALSE(error);
			int val = get_data_message(std::move(tm));
			EXPECT_TRUE(val == 1 || val == 2);
			all_do.set_value();
		});
		while(!startRead) {
			std::this_thread::sleep_for(std::chrono::milliseconds(1));
		}
		th_server.read(IOMode::Async);
		all_do.get_future().wait();
		server_socket_2->close();
	});

	client_1.join();
	client_2.join();
	server_socket_1.join();
	server_socket_2.join();
}