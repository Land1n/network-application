...
LogEvent::LogEvent(LogLevel lvl, const std::string& name_logger, const std::string& func, const std::string& msg) :
    level(lvl),
...
std::string LogEvent::formatEvent() const
{
	...
	return (boost::format("%s[%s] [%-8s] [%5d] [%-12s] [%-50s] %s%s") % color % formatTime(time) %
	        levelToString(level) % threadId % name_logger % function % message % resetColor)
	    .str();
}
