...
TransportHandler::TransportHandler(std::shared_ptr<tcp::socket> socket, uint32_t magicNumber) :
...
TransportMessage TransportHandler::read()
{
	...
	try {
		...
		if(onReadHandler)
			onReadHandler(0, transport_message.payload.data(), transport_message.payload.size());
...
bool TransportHandler::write(TransportMessage& message)
{
	...
	if(onWriteHandler)
		onWriteHandler(message.payload);
