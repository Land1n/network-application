...
void Worker::changeStatusWorker(const StatusWorker s)
{
	if(status != s) {
		...
		case StatusWorker::Wait:
			logger.log(LogLevel::Trace, __func__, "StatusWorker = Wait");
			break;
		case StatusWorker::Work:
			logger.log(LogLevel::Trace, __func__, "StatusWorker = Work");
			break;
		case StatusWorker::Stop:
			logger.log(LogLevel::Trace, __func__, "StatusWorker = Stop");
			break;
		case StatusWorker::Flush:
			logger.log(LogLevel::Trace, __func__, "StatusWorker = Flush");
			break;
		}
	}
}
