...
static std::string getSyncOrAsync(IOMode mode)
{
...
Session::Session(boost::asio::io_context& io, IOMode mode) : mode(mode)
{
	...
	connection_handler = std::make_unique<ConnectHandler>(socket);
...
{
	disconnect();
	ErrorHandler::check_error(error_code(), "Session::~Session");
}
...
void Session::connect(const std::string& address, PortInt port)
{
...
void Session::setOnConnect(const CallbackError& callback)
{
	on_connect = callback;
}

void Session::setOnAccept(const CallbackError& callback)
{
	on_accept = callback;
}

void Session::setOnDisconnect(const CallbackError& callback)
{
	on_disconnect = callback;
}
void Session::setOnReadHandler(const Network::Server::ReadHandler& handler)
{
...
void Session::setOnError(const CallbackError& callback)
{
	transport_handler->setOnError(callback);
	connection_handler->setOnError(callback);
}

void Session::setMagicNumber(uint32_t magicNumber)
{
	transport_handler->setMagicNumber(magicNumber);
}
...
void Session::setOnAllWrite(const CallbackError& callback)
{
